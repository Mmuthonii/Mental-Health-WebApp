...
<td><button class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure you want to accept this appointment?')?window.location.href='insertion.inc.php?action=acceptA&id=<?php echo($row["Appointment_ID"]); ?>':true;" title='Accept Appointment'>Accept</button></td>
<td><button class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure you want to cancel this appointment?')?window.location.href='insertion.inc.php?action=cancelA&id=<?php echo($row["Appointment_ID"]); ?>':true;" title='Cancel Appointment'>Cancel</button></td>
<td><button class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure you want to complete this appointment?')?window.location.href='insertion.inc.php?action=completeA&id=<?php echo($row["Appointment_ID"]); ?>':true;" title='Complete Appointment'>Complete</button></td>
</tr>
<?php
}
...
$sql = "SELECT `Report_ID`, `Details`, `Created_At` FROM `progress_report` WHERE `Specialist_ID` = '$filter'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
...
<td><button class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure you want to delete this progress report?')?window.location.href='insertion.inc.php?action=deleteP&id=<?php echo($row["Report_ID"]); ?>':true;" title='Delete Progress Report'>Delete</button></td>
</tr>
<?php
}
} else { echo "No results"; }
...
$sql = "SELECT `Resource_ID`, `Name`, `Details`, `File` FROM `resources` WHERE `Specialist_ID` = '$filter'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
...
<td><?php echo($row["Details"]); ?></td>
<td><button class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure you want to delete this resource?')?window.location.href='insertion.inc.php?action=deleteR&id=<?php echo($row["Resource_ID"]); ?>':true;" title='Delete Resource'>Delete</button></td>
</tr>
<?php
}
} else { echo "No results"; }
  ...
  <section id="contact" class="section-padding">
    <div class="container">
      <div class="row">
        ...
        <div class="col-md-6 col-sm-8 marb20">
          <div class="contact-info">
            <h3 class="cnt-ttl">Add A Resource</h3>
            <div class="space"></div>
            <form action="insertion.inc.php" enctype="multipart/form-data" method="post" class="contactForm">
              <div class="form-group">
                <input type="text" name="fname" class="form-control br-radius-zero" id="fname" placeholder="Resource Name" required />
              </div>
              <div class="form-group">
                <input type="file" class="form-control br-radius-zero" name="file" id="file" accept=".doc, .pdf, .png, .jpg, .mp3" required />
              </div>
              <div class="form-group">
                <select class="form-control br-radius-zero" name="uid" required>
              ...
              <div class="form-action">
                <button type="submit" class="btn btn-form" name="addre">Add</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div class="col-md-6 col-sm-8 marb20">
          <div class="contact-info">
            <h3 class="cnt-ttl">Add A Progress Report</h3>
            <div class="space"></div>
            <form action="insertion.inc.php" method="post" class="contactForm">
              <div class="form-group">
                <select class="form-control br-radius-zero" name="uid" required>
              ...
              <div class="form-action">
                <button type="submit" class="btn btn-form" name="addr">Add</button>
              </div>
            </form>
          </div>
        </div>
